    ...
    <>
      <style>{`
        ...
        @media (min-width: 1024px) {
          .card__title { font-size: 1.25rem; letter-spacing: 0.01em; }
          .card__description { font-size: 1rem; line-height: 1.6; }
        }

        /* Mobile compact mode */
        @media (max-width: 639px) {
          .card { min-height: 160px; padding: 0.75rem; border-radius: 14px; }
          .card__label { display: none; }
          .card__title { font-size: 1rem; }
          .card__description { font-size: 0.85rem; line-height: 1.35; display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 2; overflow: hidden; }
        }
        .card--border-glow::after {
          ...
          mask-composite: subtract; -webkit-mask-composite: xor; pointer-events: none; z-index: 1;
        }
        .card--border-glow:hover::after { opacity: 1; }

        /* Text clamp helpers (no plugin needed) */
      ...
      <BentoCardGrid gridRef={gridRef}>
        <div className="card-responsive grid gap-4">
          {cardData.map((card, index) => {
            const baseClassName = `card flex flex-col justify-between relative min-h-[220px] w-full max-w-full p-6 sm:p-3 rounded-[20px] border border-solid font-light overflow-hidden transition-all duration-300 ease-in-out hover:-translate-y-0.5 hover:shadow-[0_8px_25px_rgba(0,0,0,0.15)] ${enableBorderGlow ? 'card--border-glow' : ''}`;
            const cardStyle: React.CSSProperties = { backgroundColor: card.color || 'var(--background-dark)', borderColor: 'var(--border-color)', color: 'var(--white)' };
              ...
              return (
                ...
                >
                  ...
                  <div className="card__content flex flex-col relative text-white">
                    ...
                    <p className={`card__description text-xs leading-5 opacity-80 ${textAutoHide ? 'text-clamp-2' : ''}`}>{card.description}</p>
                  </div>
                </ParticleCard>
              );
            }
